...
class phpIp2Country {
	...
	function __construct($ip,$dbConfig=array()){
		...
		$this->ipInfoArr = $this->dbGetRow_mysqli($this->getIpSelectSQL(), $this->db);
		
		if(!$this->ipInfoArr){
			//die ('Error during reciving informations about IP address!');
			
			//$this->ipInfoArr['IP_STR'] = "IP_STR";
			//$this->ipInfoArr['IP_VALUE'] = "IP_VALUE";
			//$this->ipInfoArr['IP_FROM_STR'] = "IP_FROM_STR";
			//$this->ipInfoArr['IP_TO_STR'] = "IP_TO_STR";
	...
	private function dbConnect(){
		if(is_array($this->dbConfig)){
			...
			// DEPRECATED MYSQL BASIC
			//$this->db = mysql_connect($this->dbConfig['host'].':'.$this->dbConfig['port'], $this->dbConfig['dbUserName'], $this->dbConfig['dbUserPassword']);
			
			// SQLI BASIC MODE
			//$this->db = new mysqli($this->dbConfig['host'], $this->dbConfig['dbUserName'], $this->dbConfig['dbUserPassword'], $this->dbConfig['dbName']);
			
			$timeout = 3;
			$this->db= mysqli_init( );
			$this->db->options( MYSQLI_OPT_CONNECT_TIMEOUT, $timeout ) ;
$this->db->real_connect($this->dbConfig['host'], $this->dbConfig['dbUserName'] , $this->dbConfig['dbUserPassword'], $this->dbConfig['dbName']) ;
...
//}
	...
	private function dbGetRow_mysqli($sql, $mysqli){
